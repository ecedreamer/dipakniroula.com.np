...
pub async fn session_middleware(State(state): State<AppState>, req: Request, next: Next) -> Response {
    ...
    if let Some(sid) = session_id {
        ...
        if let Some(session) = get_session_by_id(pool, sid).await {
            ...
            if current_time < session.expires_at && session.role == "admin" {
                let mut req = req;
    ...
    tracing::info!("Admin authentication required. Redirecting to login.");
    Redirect::to("/auth/login").into_response()
}

pub async fn user_session_middleware(State(state): State<AppState>, req: Request, next: Next) -> Response {
    let session_id = get_session_id_from_req(&req);
    if let Some(sid) = session_id {
        let pool = state.db_pool.clone();
        if let Some(session) = get_session_by_id(pool, sid).await {
            let current_time = Utc::now().naive_utc();
            if current_time < session.expires_at && session.role == "user" {
                let mut req = req;
                req.extensions_mut().insert(session);
                return next.run(req).await;
            } else {
...
pub async fn optional_session_middleware(State(state): State<AppState>, req: Request, next: Next) -> Response {
    let session_id = get_session_id_from_req(&req);
    let mut req = req;
    if let Some(sid) = session_id {
        let pool = state.db_pool.clone();
...
async fn get_session_by_id(pool: crate::db::DbPool, session_id: String) -> Option<CustomSession> {
...
pub async fn security_headers_middleware(req: axum::extract::Request, next: Next) -> Response {
    ...
    let csp = "default-src 'self'; \
               ...
               connect-src 'self' https://www.google-analytics.com https://region1.google-analytics.com https://oauth2.googleapis.com https://www.googleapis.com; \
